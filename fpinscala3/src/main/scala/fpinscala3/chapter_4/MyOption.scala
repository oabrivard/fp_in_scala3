...
enum MyOption[+A]:
  ...
  def flatMap[B](f: A => MyOption[B]): MyOption[B] = this.map(f).getOrElse(MyOption.None)
    /*
    o match
  ...
  def filter(f: A => Boolean): MyOption[A] = this.flatMap(a => if f(a) then MyOption.Some(a) else MyOption.None)

  def orElse[B>:A](ob: => MyOption[B]): MyOption[B] = this.map(MyOption.Some(_)).getOrElse(ob)
    /*
    o match
